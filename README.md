...
### 3. 쿼리문정의 파일 구성
쿼리문정의 파일 을 생성하여 이관할 쿼리를 정의하세요. `--query` 또는 `-q` 옵션으로 파일 경로를 지정하세요.
...
#### 3. 개별 기능 실행
```bash
validate-config.bat     # 쿼리문정의 파일 검증만
test-connection.bat     # 데이터베이스 연결 테스트만
```

#### 4. XML 쿼리문정의 파일 테스트 (🆕 신규)
```bash
test-xml-migration.bat
```
- XML 형식의 쿼리문정의 파일 을 사용한 완전한 테스트
...
- 쿼리문정의 파일 의 `variables` 섹션에 정의된 변수
- 환경 변수 (예: `${BATCH_SIZE}`)
...
각 작업 유형별로 별도의 쿼리문정의 파일 을 관리하여 체계적인 데이터 이관이 가능합니다.
...
프로젝트에서는 JSON과 XML 두 가지 형식의 쿼리문정의 파일 을 모두 지원합니다.
...
- **환경별 분리**: 개발/스테이징/운영 환경별로 다른 쿼리문정의 파일 사용
- **버전 관리**: 쿼리문정의 파일 을 Git으로 관리하여 DB 설정 이력 추적
- **팀 협업**: 팀원 간 동일한 DB 설정 공유
- **설정 통합**: 쿼리와 DB 설정을 하나의 파일에서 관리

### 보안 고려사항

⚠️ **주의**: 쿼리문정의 파일에 DB 비밀번호를 포함할 때는 다음 사항을 고려하세요:

- **개발 환경**: 쿼리문정의 파일에 직접 포함 가능
- **운영 환경**: 환경 변수나 별도 보안 저장소 사용 권장
- **Git 관리**: `.gitignore`에 민감한 쿼리문정의 파일 추가 고려
- **권한 관리**: 쿼리문정의 파일 접근 권한 적절히 제한
...
5. **SQL 파일 경로**: 상대 경로는 쿼리문정의 파일 기준으로 해석됩니다.
