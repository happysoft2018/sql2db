...
echo [정보] 설정 파일: queries/migration-queries.xml
echo.

:: 1. 설정 파일 검증
echo [1단계] 설정 파일 검증 중...
node src/migrate-cli.js validate --config queries/migration-queries.xml
if errorlevel 1 (
    echo.
...
if /i "!confirm!"=="Y" (
    ...
    node src/migrate-cli.js migrate --config queries/migration-queries.xml
